...
以上图来自于MDN。总结一下：乘除>加减>按位左移右移>比较运算>与或异或。
...
我们现在先来考虑一个非负整数的二进制长度。在JS中，我们如何计算一个数转化为二进制之后，它的长度是多少？也就是说我们把它叫成二进制长度大小，我们需要用到`Math.clz32()`方法，"clz32" 是 CountLeadingZeroes32 的缩写。：
