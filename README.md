...
在 JavaScript 中，普通的数字（Number 类型）是基于 IEEE 754 双精度浮点数格式存储的，这导致它们只能安全地表示 -9007199254740991（即 -(2^53 - 1)）到 9007199254740991（即 2^53 - 1）之间的整数。而 BigInt 就解决了这个问题。
...
## 1.12 LeetCode 304 二维区域和检索-矩阵不可变 二维前缀和
